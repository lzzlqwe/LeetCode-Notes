...
class Solution {
    public ListNode reverseKGroup(ListNode head, int k) {
        ...
        ListNode curLeft = preHead; //当前反转链表段的上一节点
        //之后计算总共需要反转多少段链表
        for(int i = 0; i < n/k; i++){
            cur = curLeft.next;
            ListNode post = cur, pre = null;
            //反转当前段的链表
            for(int j = 0; j < k; j++){
                ListNode nxt = cur.next;
                cur.next = pre;
...
### 方法二（堆排序）
思路：堆排序，首先将数组转换为最大堆，然后依次取出最大值并调整堆。详见代码思路。[代码解析](https://www.doubao.com/thread/wa8b6931f9713f5e9)      
代码：
```
class Solution {
    public int[] sortArray(int[] nums) {
